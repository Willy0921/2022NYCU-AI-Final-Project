...
parser.add_argument("--file", type=str, default="Qlearning",
    ...
    def learn(self, state, action, reward, next_state, done, count):
        ...
        if done:
            if np.mean(max_count) <= count: 
                np.save("./Train_data/Qlearning/tables/" + args.file + ".npy", self.qtable)
...
def test(env):
    ...
    testing_agent.qtable = np.load("./Train_data/Qlearning/tables/" + args.file + ".npy", allow_pickle = True).item()
    rewards = []
...
if __name__ == "__main__":
    ...
    np.save("./Train_data/Qlearning/rewards/" + args.file + ".npy", np.array(total_reward))

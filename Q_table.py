...
parser.add_argument("--decay_frequency",type= int,      default= 20)
...
class Agent():
    def __init__(self, env, epsilon=args.epsilon, learning_rate= args.learning_rate, GAMMA=args.GAMMA, num_bins=args.num_bins):
        """
    ...
    def learn(self, state, action, reward, next_state, done, count):
        ...
        self.qtable[tuple(state)][action] = \
               self.qtable[tuple(state)][action] *(1 - self.learning_rate) \
             + self.learning_rate * (reward * args.reward_ratio + self.gamma * max(self.qtable[tuple(next_state)]))
        # End your code

        # You can add some conditions to decide when to save your table
        global max_steps
        if done:
            if np.mean(max_steps) < count: 
                np.save("./Tables/Qtable_Freeway.npy", self.qtable)

            inx = np.argmin(max_steps)
            if max_steps[inx] < count: max_steps[inx] = count
...
def test(env):
    ...
    testing_agent.qtable = np.load("./Tables/Qtable_Freeway.npy")
    rewards = []
